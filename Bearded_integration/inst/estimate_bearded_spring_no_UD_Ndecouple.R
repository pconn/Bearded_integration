...
TmbFile1 = "C:/Users/paul.conn/git/Bearded_integration/Bearded_integration/src/fit_bearded_real_noUD_Ndecouple"
...
gam_setup = gam(Dummy ~ s(depth, bs = "cs",k=4) + s(dist_land, bs = "cs",k=4) + 
                  s(easting, bs = "cs",k=9) + s(northing,bs="cs",k=9),
...
#X_s = gam_setup$X[,-1]
X_s = matrix(0,n_cells,11)  #use fixed effects set-up for debugging 
X_s[,1]=grid_sf$depth
...
X_s[,7]=grid_sf$easting
X_s[,8]=grid_sf$easting^2
X_s[,9]=grid_sf$easting*grid_sf$northing
X_s[,10]=grid_sf$easting^2*grid_sf$northing
X_s[,11]=grid_sf$easting*grid_sf$northing^2
...
Obj = MakeADFun( data=Data, parameters=Params, random=Random, map=Map, DLL="fit_bearded_real_noUD_Ndecouple",silent=FALSE)
Obj$fn( Obj$par )
