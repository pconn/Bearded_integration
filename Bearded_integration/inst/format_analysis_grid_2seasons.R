...
AK = read_sf('c:/users/paul.conn/work/git/BeaufortPower/shapefiles/ak_dcw.shp')
AK = st_transform(AK,st_crs(grid_sf))
Russia = read_sf('c:/users/paul.conn/work/git/BeaufortPower/shapefiles/russia_dcw.shp')
...
depth[which(depth$gridcode>0),"gridcode"]=0  #set depths on land = 0
...
depth$mean = apply(cbind(depth$gridcode,depth$maxbindept),1,'mean')  #using midpoint between min and max depth for each polygon
grid_sf$depth = NA
...
load('c:/users/paul.conn/git/JOBSS/JOBSS_ests.RData')
...
Est_CHESS$meta$grid$Bd_SE = sqrt(Est_CHESS$var_infl*Est_CHESS$var_infl_mvn*diag(Est_CHESS$bd_vc))
...
Est_wBS_2012$meta$grid$Bd_SE = sqrt(Est_wBS_2012$var_infl*Est_wBS_2012$var_infl_mvn*(diag(Est_wBS_2012$bd_vc)+0.0004)) # a couple entries are slightly negative
...
Est_wBS_2013$meta$grid$Bd_SE = sqrt(Est_wBS_2013$var_infl*Est_wBS_2013$var_infl_mvn*(diag(Est_wBS_2013$bd_vc)+0.0001)) # a couple entries are slightly negative
...
Est_eBS_2012$meta$grid$Bd_SE = sqrt(Est_eBS_2012$var_infl*Est_eBS_2012$var_infl_mvn*(diag(Est_eBS_2012$bd_vc)+0.0001)) # a couple entries are slightly negative
...
Est_eBS_2013$meta$grid$Bd_SE = sqrt(Est_eBS_2013$var_infl*Est_eBS_2013$var_infl_mvn*(diag(Est_eBS_2013$bd_vc)+0.0001)) # a couple entries are slightly negative
...
#Beaufort
JOBSS_out$Grid$Bd = JOBSS_out$Z_s
var_infl = 1.54  #(ratio of total variance to estimation-only variance)
var_infl_mvn = 36.3  #ratio of hessian variance of N to sum(var(N_s)) [correction for assuming N_s are independent]
JOBSS_out$Grid$Bd_SE = sqrt(var_infl*var_infl_mvn*(JOBSS_out$SE_s^2+0.0001)) 

Which_in = which(st_within(st_centroid(JOBSS_out$Grid),st_union(grid_sf),sparse=FALSE))
JOBSS_out$Grid=JOBSS_out$Grid[Which_in,]  #get rid of cells outside bearded integration grid
Within = st_within(st_centroid(JOBSS_out$Grid),grid_sf,sparse=FALSE)
Mapping_JOBSS = rep(0,nrow(JOBSS_out$Grid))  #this object will be needed for integrated modeling too
for(icell in 1:nrow(JOBSS_out$Grid))Mapping_JOBSS[icell]=which(Within[icell,]==1)
grid_sf$bd_JOBSS = grid_sf$bd_JOBSS_SE = NA
grid_sf$bd_JOBSS[Mapping_JOBSS]=JOBSS_out$Grid$Bd
grid_sf$bd_JOBSS_SE[Mapping_JOBSS] = JOBSS_out$Grid$Bd_SE 
...
plot_JOBSS = ggplot(grid_sf)+geom_sf(aes(fill=bd_JOBSS,colour=bd_JOBSS))+
  scale_fill_viridis(name="Abundance",na.value="transparent",limits=c(0,100)) + 
  scale_color_viridis(name="Abundance",na.value="lightgray",limits=c(0,100)) +
  ggtitle("D. Beaufort 2021")


png("Abundance_maps.png")
  plot_grid(plot_BOSS_2012,plot_BOSS_2013,plot_Chukchi,plot_JOBSS,ncol=2)
dev.off()
...
Mapping_JOBSS = which(! is.na(grid_sf$bd_JOBSS))
grid_sf$JOBSS_smoothed = grid_sf$bd_JOBSS
grid_sf$JOBSS_smoothed[which(is.na(grid_sf$JOBSS_smoothed))]=0
grid_sf$JOBSS_smoothed = Probs %*% grid_sf$JOBSS_smoothed
grid_sf$JOBSS_smoothed[which(is.na(grid_sf$bd_JOBSS))]=NA #reset cells in different sea to NA
SE_JOBSS = diag(Probs[Mapping_JOBSS,Mapping_JOBSS] %*% 
                    Diagonal(x=grid_sf$bd_JOBSS_SE[Mapping_JOBSS]) %*% 
                    t(Probs[Mapping_JOBSS,Mapping_JOBSS]))
grid_sf$JOBSS_smoothed_SE = rep(NA,n_cells) 
grid_sf$JOBSS_smoothed_SE[Mapping_JOBSS]=SE_JOBSS
...
plot_JOBSS_smoothed =ggplot(grid_sf)+geom_sf(aes(fill=JOBSS_smoothed,color=JOBSS_smoothed))+
  scale_fill_viridis(name="Abundance",na.value="transparent",limits=c(0,100)) + 
  scale_color_viridis(name="Abundance",na.value="lightgray",limits=c(0,100)) + 
  ggtitle("G. Beaufort 2021 - smoothed")


png("Abundance_maps2.png",width=8,height=8,units="in",res=1200)
plot_grid(plot_BOSS_2012,plot_BOSS_2012_smoothed,plot_BOSS_2013,
          plot_BOSS_2013_smoothed,plot_Chukchi,plot_Chukchi_smoothed,
          plot_JOBSS,plot_JOBSS_smoothed,ncol=2)
dev.off()
...
for(iyr in 1:n_yrs){
  for(iseason in 1:n_seasons){
    if(iseason==1)date1 = paste0(iyr+start_yr-1,'-',season_start[iseason])
    else date1 = paste0(iyr+start_yr,'-',season_start[iseason])
