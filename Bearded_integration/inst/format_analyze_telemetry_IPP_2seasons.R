...
for(is in 1:n_seasons){
  if(is==1)Which = which(Month %in% c(12,1:5))
  if(is==2)Which = which(Month %in% c(6:11))
...
Data_list[[1]]$season = factor(Data_list[[1]]$season, levels=c('1','2'))
Cur_data$ind=1
for(iind in 2:n_ind){
  Tmp = Telem[[1]][[iind]]
  Tmp$ind = iind
  ...
  Data_list[[iind]]$season = factor(Data_list[[iind]]$season, levels=c('1','2'))
}
Cur_data$season=1
Data = Cur_data
prev_ind = iind
...
for(is in 2:n_seasons){
  ...
  Data_list[[prev_ind]]$season = factor(Data_list[[prev_ind]]$season, levels=c('1','2'))
...
save(Data,file="telem_counts_2season.RData")
save(Data_list,file="telem_counts_list_2season.RData")
save.image("telem_analysis_workspace_2season.RData")



load("telem_analysis_workspace_2season.RData")
...
logUD_output <- list(logUD = Report$log_UD_s, SE = matrix(SD$sd[1:(n_cells*t_steps)],n_cells,t_steps))
save(logUD_output,file="logUD_output_2seasons.RData")

grid_sf$crap = logUD_output$logUD[,29]
plot(grid_sf[,"crap"])
...
grid_sf_big$RelAbund=exp(Report$log_UD_s[,year*2+1])/sum(exp(Report$log_UD_s[,year*2+1]))
...
grid_tmp$Year = "2012"
grid_tmp$RelAbund=exp(Report$log_UD_s[,year*2+is])/sum(exp(Report$log_UD_s[,year*2+is]))
...
png("UD_plots_2seasons.png",width=8,height=8,units="in",res=1200)
  Plot
dev.off()
