...
n_Bering = length(SE_2012)
N_2012 = sum(Bering_2012_N)
logN_2012 = log(N_2012)
varN = sum(diag(Bering_2012_VC))+2*sum(lower.tri(Bering_2012_VC,diag=FALSE))
logN_var_2012 = 1/N_2012^2 * varN #delta method
Pi_2012 = Bering_2012_N/sum(Bering_2012_N)
Var_Pi = (diag(Bering_2012_VC) - Pi_2012^2*varN)/(varN+N_2012^2)
...
#N and Pi parameterization
n_Bering = length(SE_2013)
N_2013 = sum(Bering_2013_N)
logN_2013 = log(N_2013)
varN = sum(diag(Bering_2013_VC))+2*sum(lower.tri(Bering_2013_VC,diag=FALSE))
logN_var_2013 = 1/N_2013^2 * varN #delta method
Pi_2013 = Bering_2013_N/sum(Bering_2013_N)
Var_Pi = (diag(Bering_2013_VC) - Pi_2013^2*varN)/(varN+N_2013^2)
...
#the following sets N to truth if uncommented
#Chukchi_N = N_s[which_Chukchi,(2016-2004)*5+2]

#N and Pi parameterization
n_Chukchi = length(SE_2016)
N_2016 = sum(Chukchi_N)
logN_2016 = log(N_2016)
varN = sum(diag(Chukchi_VC))+2*sum(lower.tri(Chukchi_VC,diag=FALSE))
logN_var_2016 = 1/N_2016^2 * varN #delta method
Pi_2016 = Chukchi_N/sum(Chukchi_N)
Var_Pi = (diag(Chukchi_VC) + nugget - Pi_2016^2*varN)/(varN+N_2016^2)
...
TmbFile1 = "C:/Users/paul.conn/git/Bearded_integration/Bearded_integration/src/fit_bearded_sim_all_Ndecouple"
...
wt_ud = 0.000001  #above 1 increases ud influence
wt_ckmr = 1  #above 1 decreases ckmr influence
wt_aerial = 1 #above 1 increases aerial influence
wt_acc=0.000001  # above 1 increases acoustic influence
wt_spline =1
...
Data<- list("Acc_k"=as.matrix(Acc_det_sum[,c(2,4,6,8,10)]), 
            ...
            "Nst_2012" = Bering_2012_N, "Nst_2013"=Bering_2013_N, "Nst_2016"=Chukchi_N,
            ...
            "Wts" = c(wt_ud, wt_ckmr, wt_aerial, wt_acc, wt_spline, wt_aerial_N),"n_tsteps"=n_season,
...
Obj = MakeADFun( data=Data, parameters=Params, random=Random, map=Map, DLL="fit_bearded_sim_all_Ndecouple",silent=FALSE)
Obj$fn( Obj$par )
