...
n_sims = 100
N_est = Converge = matrix(NA,3,n_sims)  #result for aerial only, kitchen sink equal wt, kitchen sink iterative rw
...
for(iseason in 1:n_season){
  ...
  Var_UD = Pi_UD*(1-Pi_UD)/(1000+1) #implies dirichlet alpha0 = 1000
...
TmbFile1 = "C:/Users/paul.conn/git/Bearded_integration/Bearded_integration/src/fit_bearded_sim_all_dirichlet"
...
#impose minimum value for Pi_UD = 0.00000001
Pi_UD[which(Pi_UD<0.00000001)]=0.00000001
for(it in 1:n_season)Pi_UD[,it] = Pi_UD[,it] / sum(Pi_UD[,it])
...
Obj = MakeADFun( data=Data, parameters=Params, random=Random, map=Map, DLL="fit_bearded_sim_all_dirichlet",silent=FALSE)
Obj$fn( Obj$par )
...
n_MC_sims = 100 #number of monte carlo simulations to get a p-value for dirichlet 
n_MC_opt = 10
MC_SSQ = rep(0,100)
Sim_data = matrix(0,n_cells,n_season)
dc_cur = Data$dirichlet_conc
Sim_res = matrix(0,2,n_MC_opt)
for(i in 1:n_iter){
  # change dirichlet precision until likelihood based p-value is between 0.2 and 0.8
  Pi_cur = Report$Pi_st*dc_cur
  Pi_cur[which(Pi_cur<0.00000000001)]=0.00000000001
  for(it in 1:n_season)Pi_std[,it] = Pi_cur[,it]/sum(Pi_cur[,it])
  for(imc in 1:n_MC_sims){
    for(it in 1:n_season)Sim_data[,it]=as.numeric(gtools::rdirichlet(1,Pi_cur[,it]))
    MC_SSQ[imc] = sum((Pi_std-Sim_data)^2)
  }
  SSQ_real = sum((Pi_std-Data$UD)^2)
  ECDF = ecdf(MC_SSQ)
  cur_pval = ECDF(SSQ_real)
  Sim_res[,1] = c(dc_cur,cur_pval)
  sim_counter = 2
  while((cur_pval<0.2 | cur_pval>0.8) & sim_counter<=n_MC_opt){
    if(cur_pval<0.2)dc_cur = dc_cur*1.1
    if(cur_pval>0.8)dc_cur = dc_cur*0.9
    Pi_cur = Report$Pi_st*dc_cur
    Pi_cur[which(Pi_cur<0.00000000001)]=0.00000000001
    for(it in 1:n_season)Pi_std[,it] = Pi_cur[,it]/sum(Pi_cur[,it])
    for(imc in 1:n_MC_sims){
      for(it in 1:n_season)Sim_data[,it]=gtools::rdirichlet(1,Pi_cur[,it])
      MC_SSQ[imc] = sum((Pi_std-Sim_data)^2)
    }
    SSQ_real = sum((Pi_std-Data$UD)^2)
    ECDF = ecdf(MC_SSQ)
    cur_pval = ECDF(SSQ_real)
    Sim_res[,sim_counter] = c(dc_cur,cur_pval)
    sim_counter = sim_counter+1 
  }
  Data$dirichlet_conc = dc_cur
