...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_SCALAR(logN_2012); //log abundance estimate for Bering 2012 
  DATA_SCALAR(logN_2013);//log abundance estimate for Bering 2013 
  DATA_SCALAR(logN_2016);//log abundance estimate for Chukchi 2016
  DATA_SCALAR(logN_se_2012); //log abundance estimate for Bering 2012 
  DATA_SCALAR(logN_se_2013);//log abundance estimate for Bering 2013 
  DATA_SCALAR(logN_se_2016);//log abundance estimate for Chukchi 2016
  DATA_VECTOR(Pi_mean_adj_2012); //mean-adjusted rel abundance values for Bering in 2012 (sum = num grid cells surveyed)
  DATA_VECTOR(Pi_mean_adj_2013);
  DATA_VECTOR(Pi_mean_adj_2016);
  DATA_VECTOR( Nst_2012 );  //abundance values predicted for each Bering grid cell in 2012 (for chi-sq)
  DATA_VECTOR( Nst_2013 );  //abundance values predicted for each Bering grid cell in 2012 (for chi-sq)
  DATA_VECTOR( Nst_2016 );  //abundance values predicted for each Bering grid cell in 2012 (for chi-sq)
  ...
  // global stuff
  vector<Type> jnll_comp(6);  //neg. log likelihood components: UD, CKMR, Aerial rel abund, Acoustics, spline prior, aerial abs abund
  jnll_comp.setZero();
  Type jnll = 0.0;
  Type N = exp(log_N);
  ...
  vector<Type> E_Pi_mean_adj_2012(n_bering);
  vector<Type> E_Pi_mean_adj_2013(n_bering);
  vector<Type> E_Pi_mean_adj_2016(n_chukchi);
  Type Exp_log_N_2012 = 0.0;
  Type Exp_log_N_2013 = 0.0;
  Type Exp_log_N_2016 = 0.0;
  ...
  int t_step3 = 61;  //corresponds to model time step associated with april/may 2016
  
  Type sum_2012 = 0.0;
  Type sum_2013 = 0.0;
  Type sum_2016 = 0.0;
  for(int i=0; i<n_bering; i++){
    Exp_log_N_2012 += Z_st(which_Bering(i),t_step1);
    Exp_log_N_2013 += Z_st(which_Bering(i),t_step2);
    E_Pi_mean_adj_2012(i)=Pi_st(which_Bering(i),t_step1);
    E_Pi_mean_adj_2013(i)=Pi_st(which_Bering(i),t_step2);
    sum_2012 += E_Pi_mean_adj_2012(i);
    sum_2013 += E_Pi_mean_adj_2013(i);
  }
  Exp_log_N_2012 = log(Exp_log_N_2012+small);
  Exp_log_N_2013 = log(Exp_log_N_2013+small);
  E_Pi_mean_adj_2012 = E_Pi_mean_adj_2012 * n_bering / sum_2012;
  E_Pi_mean_adj_2013 = E_Pi_mean_adj_2013 * n_bering / sum_2013;
  for(int i=0; i<n_chukchi; i++){
    Exp_log_N_2016 += Z_st(which_Chukchi(i),t_step3);
    E_Pi_mean_adj_2016(i)=Pi_st(which_Chukchi(i),t_step3);
    sum_2016 += E_Pi_mean_adj_2016(i);
  }
  Exp_log_N_2016 = log(Exp_log_N_2016+small);
  E_Pi_mean_adj_2016 = E_Pi_mean_adj_2016 * n_chukchi / sum_2016;
  
  
  for(int i=0; i<n_bering; i++){  //wLS obj function, chisq for GOF
    chisq_aerial += pow(Nst_2012(i)-Z_st(which_Bering(i),t_step1),2.0)/Z_st(which_Bering(i),t_step1);
    chisq_aerial += pow(Nst_2013(i)-Z_st(which_Bering(i),t_step2),2.0)/Z_st(which_Bering(i),t_step2);
    jnll_comp(2) += W_2012(i)*pow(Pi_mean_adj_2012(i)-E_Pi_mean_adj_2012(i),2.0);
    jnll_comp(2) += W_2013(i)*pow(Pi_mean_adj_2013(i)-E_Pi_mean_adj_2013(i),2.0);
  }
  for(int i=0; i<n_chukchi; i++){
    chisq_aerial += pow(Nst_2016(i)-Z_st(which_Chukchi(i),t_step3),2.0)/Z_st(which_Chukchi(i),t_step3);
    jnll_comp(2) += W_2016(i)*pow(Pi_mean_adj_2016(i)-E_Pi_mean_adj_2016(i),2.0);
  }
  jnll_comp(2)=Wts(2)*jnll_comp(2);  //aerial rel abundance
  
  //now, aerial abs abund
  jnll_comp(5) -= dnorm(logN_2012,Exp_log_N_2012,Wts(5)*logN_se_2012,1);  //log likelihood - normal on log-scale
  jnll_comp(5) -= dnorm(logN_2013,Exp_log_N_2013,Wts(5)*logN_se_2013,1);  //log likelihood - normal on log-scale
  jnll_comp(5) -= dnorm(logN_2016,Exp_log_N_2016,Wts(5)*logN_se_2016,1);  //log likelihood - normal on log-scale
  ...
  REPORT(log_N);
  REPORT(N);
  REPORT(E_Pi_mean_adj_2012);
  REPORT(E_Pi_mean_adj_2013);  
  REPORT(E_Pi_mean_adj_2016);  
  REPORT(N_acc);
  REPORT(Prob_acc);
  REPORT(Beta_acc);
  REPORT(chisq_aerial);
  REPORT(chisq_acc);
  REPORT(Chisq_acc);
  REPORT(Exp_log_N_2012);
  REPORT(Exp_log_N_2013);
  REPORT(Exp_log_N_2016);
