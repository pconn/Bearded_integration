...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_MATRIX(UD); //each column gives dirichlet concentration params for each time step
  DATA_SCALAR(dirichlet_conc);
  ...
  matrix<Type> Pi_st_dirichlet = Pi_st*dirichlet_conc;
  for(int it=0; it<n_tsteps; it++){
    for(int is=0; is<n_cells; is++){
      jnll_comp(0)-=(Pi_st_dirichlet(is,it)-1.0)*log(UD(is,it));
  ...
  for(int i=0; i<n_bering; i++){
    ...
    if(MVN!=1){
      ...
      //jnll_comp(2) -= dpois_kern_log2(N_2012(i),Z_st(which_Bering(i),t_step1)); 
      //jnll_comp(2) -= dpois_kern_log2(N_2013(i),Z_st(which_Bering(i),t_step2)); 
      //jnll_comp(2) -= dtweedie( N_2012(i), Z_st(which_Bering(i),t_step1),phi(0),power(0), true );
      //jnll_comp(2) -= dtweedie( N_2013(i), Z_st(which_Bering(i),t_step2),phi(1),power(1), true );
    }
  }
  for(int i=0; i<n_chukchi; i++){
    ...
    if(MVN!=1){
      ...
      //jnll_comp(2) -= dtweedie( N_2016(i), Z_st(which_Chukchi(i),t_step3),phi(2),power(2), true );
    }
